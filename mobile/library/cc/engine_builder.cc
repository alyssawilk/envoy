...
std::unique_ptr<envoy::config::bootstrap::v3::Bootstrap> EngineBuilder::generateBootstrap() const {
  ...
  // TODO(alyssawilk) handle apple resolver.
...
#include "library/common/config/no_whitespace_certificates.inc"
  ...
  (*flags.mutable_fields())["http2_delay_keepalive_timeout"].set_bool_value(
      h2_extend_keepalive_timeout_);
  ...
  bootstrap->mutable_typed_dns_resolver_config()->set_name(
      "envoy.network.dns_resolver.getaddrinfo");
  bootstrap->mutable_typed_dns_resolver_config()->mutable_typed_config()->PackFrom(resolver_config);
