...
TEST(HeaderParserTest, EvaluateHeaderValuesWithNullStreamInfo) {
  ...
  HeaderParserPtr req_header_parser_set =
      HeaderParser::configure(headers_values, HeaderValueOption::OVERWRITE_IF_EXISTS_OR_ADD).value();
...
request_headers_to_remove: ["x-nope"]
  ...
  HeaderParserPtr req_header_parser =
      HeaderParser::configure(route.request_headers_to_add(), route.request_headers_to_remove()).value();
...
response_headers_to_remove: ["x-nope"]
  ...
  HeaderParserPtr resp_header_parser =
      HeaderParser::configure(route.response_headers_to_add(), route.response_headers_to_remove()).value();
...
response_headers_to_add:
  ...
  const auto route = parseRouteFromV3Yaml(yaml);
  HeaderParserPtr resp_header_parser = HeaderParser::configure(route.response_headers_to_add()).value();
  NiceMock<Envoy::StreamInfo::MockStreamInfo> stream_info;
